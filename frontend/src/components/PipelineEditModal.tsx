...
export function PipelineEditModal({
  ...
  const [tab, setTab] = useState<"weights" | "thresholds" | "escalation" | "system">("weights");
  ...
  async function handleWeightChange(category: string, weight: number) {
    try {
  ...
  async function handleToggleWeight(category: string, active: boolean) {
    try {
  ...
  const tabs = [
    ...
    { key: "escalation" as const, label: "Escalation" },
  ...
  return (
    <div className="fixed inset-0 z-50 modal-overlay flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-[#0a0a0a] border border-[#333] rounded-lg w-full max-w-2xl max-h-[80vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#333]">
          <div>
            <h2 className="text-sm font-bold tracking-wide uppercase">{sourceName}</h2>
            <p className="text-[11px] text-[#999] mt-0.5">
              Pipeline config · {mode} mode · {sourceId.slice(0, 8)}...
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Message */}
        {message && (
          <div className="mx-5 mt-3 px-3 py-1.5 rounded bg-[#1a1a1a] border border-[#444] text-[11px] text-[#aaa]">
            {message}
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <span className="text-[#999] text-xs">Loading...</span>
            </div>
          ) : (
            <>
              {/* Weights Tab */}
                ...
                <>
                    ...
                    ([category, info]: [string, any]) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={info.active}
                          onChange={(e) => handleToggleWeight(category, e.target.checked)}
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#333]">
                    <input
                      ...
                      className="flex-1 px-2.5 py-1.5 rounded bg-[#111] border border-[#333] text-xs text-white placeholder:text-[#666] focus:outline-none focus:border-[#555]"
                    />
                    <input
                      type="number"
                      min={0}
                ...
                <div className="space-y-2">
                  {rules.map((rule) => (
                    ...
                    >
                      ...
                      <span className="text-[11px] text-[#999] font-mono flex-1">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      ...
                      className="flex items-center gap-3 bg-[#111] rounded px-3 py-2.5 border border-[#2a2a2a]"
                    >
                      <span className="text-[11px] text-[#999] font-mono w-44 shrink-0">
                        {key}
                      </span>
                      <input
                        type={type}
